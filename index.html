<!DOCTYPE html>
<html lang="en">

<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <link rel="stylesheet" href="style.css">
  <title>Mifu Draperies & Curtains</title>
</head>

<body>
  <!-- Header -->
  <section id="header" onload="transparentBg">
    <div class="header container">
      <div class="nav-bar">
        <div class="brand">
          <a href="index.html">
            <img class="logo" src="./img/logo1.png" alt="logo">
          </a>
        </div>
        <div class="nav-list">
          <div class="hamburger">
            <div class="bar"></div>
          </div>
          <ul>
            <li><a href="#hero" data-after="Home">Home</a></li>
            <li><a href="#services" data-after="Service">Services</a></li>
            <li><a href="shop.html" data-after="Projects">Pruducts</a></li>
            <li><a href="corporate.html" data-after="Corporate">Corporate</a></li>
            <li><a href="#contact" data-after="Contact">Contact</a></li>
            <li><a href="#contact" data-after="Contact">Log In</a></li>
            <li><a href="cart./index.html" data-after="Cart" class="bagicon"><img class="bag" src="img/bag.png" alt="cart"></a></li>
          </ul>
        </div>
      </div>
    </div>
  </section>
  <!-- End Header -->


  <!-- Hero Section  -->
  <section id="hero">
    <div class="hero container">
      <div>
        <h1>Customize<span></span></h1>
        <h1>Curtains & Blinds<span></span></h1>
        <h1>At Your Fingertips<span></span></h1>
        <a href="shop.html" type="button" class="cta"><b>Shop Now</b></a>
      </div>
    </div>
  </section>
  <!-- End Hero Section  -->

  <!-- Service Section -->
  <section id="services">
    <div class="services container">
      <div class="service-top">
        <h1 class="section-title">Serv<span>i</span>ces</h1>
        <p><b>We provide wide range of services including finding you the most appropriate type of blinds, as well as
          product recommendation and further consultation, in order to bring out the best outcome possible that 
          would satisfy your needs.</b>
        </p>
      </div>
      <div class="service-bottom">
        <div class="service-item">
          <div class="icon"><img src="img/blinds.png"></div>
          <h2>Selection</h2>
          <p>You will have access to a variety of choice depending on your personal preference. we provide 
            different products including Drapes, Blinds, Shades.
          </p>
        </div>
        <div class="service-item">
          <div class="icon"><img src="img/maintenance.png" /></div>
          <h2>Installation</h2>
          <p>Upon request, we also provide installation service to ensure the quickest and most user-friendly
             service. <br><br></p>
        </div>
        <div class="service-item">
          <div class="icon"><img src="img/recommendation.png" /></div>
          <h2>Recommendation</h2>
          <p>Rather than purposefully chargnig you costly prices, we provide a low price, guarantee 
            year-round, provided that it is made to order from us.</p>
        </div>
        <div class="service-item">
          <div class="icon"><img src="img/conversation.png" /></div>
          <h2>Consultation</h2>
          <p>Even after all the process is over, we will make sure to <br><br><br><br> </p>
        </div>
      </div>
    </div>
  </section>
  <!-- End Service Section -->

  <!-- Contact Section -->
  <section id="contact">
    <div class="contact container" style="min-height: 30vh;">
      <div>
        <h1 class="section-title">Contact <span>info</span></h1>
      </div>
      <div class="contact-items">
        <div class="contact-item">
          <div class="icon"><a href="tel:+1 902 9890740"><img src="https://img.icons8.com/bubbles/100/000000/phone.png" /></a></div>
          <div class="contact-info">
            <a href="tel:+1 902 9890740">
            <h1>Phone</h1>
            <h2>+1 902 989 0740</h2>
            </a>
          </div>
        </div>
        <div class="contact-item">
          <div class="icon"><a href="mailto:mifudecor@gmail.com"><img src="https://img.icons8.com/bubbles/100/000000/new-post.png" /></a></div>
          <div class="contact-info">
            <a href="mailto:mifudecor@gmail.com">
            <h1>Email</h1>
            <h2>mifudecor@gmail.com</h2>
            </a>
          </div>
        </div>
        <div class="contact-item">
          <div class="icon"><img src="https://img.icons8.com/bubbles/100/000000/map-marker.png" /></div>
          <div class="contact-info">
            <h1>Address</h1>
            <h2>201 Brookline Plaza, Bedford, NS B4B 2E6</h2>
          </div>
        </div>
      </div>
    </div>
  </section>
  <!-- End Contact Section -->

  <!-- Footer -->
  <section id="footer">
    <div class="footer container">
      <div class="brand">
        <img class="bottom-logo" src="img/logo.png" alt="logo">
      </div>
      <h2>Mifu Curtains & Blinds</h2>
      <div class="social-icon">
        <div class="social-item">
          <a href="#"><img src="https://img.icons8.com/bubbles/100/000000/facebook-new.png" /></a>
        </div>
        <div class="social-item">
          <a href="#"><img src="https://img.icons8.com/bubbles/100/000000/instagram-new.png" /></a>
        </div>
        <div class="social-item">
          <a href="#"><img src="img/twitter.png" /></a>
        </div>
        <div class="social-item">
          <a href="#"><img src="https://img.icons8.com/bubbles/100/000000/behance.png" /></a>
        </div>
      </div>
      <p>Copyright © 2023 Mifu. All rights reserved</p>
    </div>
  </section>
  <!-- End Footer -->
  <script src="./app.js"></script>
</body>

</html>